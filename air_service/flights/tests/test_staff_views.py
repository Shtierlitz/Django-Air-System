...
from flights.staff_forms import FlightCreateForm, FlightUpdateForm
...
class TestStaffViews(TestCase):
    ...
    @override_settings(CELERY_TASK_ALWAYS_EAGER=True)
    @patch('flights.staff_forms.clean_or_errors')
    @patch('flights.staff_forms.flight_expiration_task.apply_async')
    def test_FlightUpdateView(self, mock_data, task_data):
        mock_data.return_value = self.flight
        task_data.return_value = None
